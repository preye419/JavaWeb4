pipeline {
    ...
    stages {
        ...
        stage('Build Application') {
            steps {
                // Wrap the build in a node block to provide workspace
                node {
                    sh 'mvn -Dmaven.compiler.source=17 -Dmaven.compiler.target=17 clean package -DskipTests'
                }
            }
        ...
        stage('Run Tests') {
            steps {
                // Wrap the test run in a node block to provide workspace
                node {
                    sh 'mvn test -e'
                }
            }
        ...
        stage('Build Docker Image') {
            steps {
                // Wrap the Docker build in a node block
                node {
        ...
        stage('Push to Docker Hub') {
            steps {
                // Wrap the Docker login and push in a node block
                node {
                    sh '''
        ...
        stage('Deploy') {
            steps {
                // Wrap the Docker deploy in a node block
                node {
                    sh '''
    ...
    post {
        always {
            // Wrap the Docker logout in a node block
            node {
